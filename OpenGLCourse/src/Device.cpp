...
auto	Device::Initialize() -> void
{
	...
	Node* tRexNode = new Node(root, "TRex Node");

	root->AddChild(tRexNode);

	ObjModel* trex = new ObjModel();
	trex->Initialize("resources/models/Trex.obj");

	tRexNode->AddModel(trex);

	Camera*	cam = new Camera(root, "Camera Node");
	cam->Translate(1.0f, -4.0f, -10.f);
...
auto	Device::Draw() const -> void
{
	auto program = _shaderMgr->GetProgram("basic");
	glUseProgram(program);
